...
{   
	...
	public static void main(String[] args) throws Exception
	{
		...
		if(t){	
			...
			boolean file_exists=false; 
				...
				alpha=Float.parseFloat(scan_string.next());
				System.out.println("Please enter the name of output file: ");
				fileName=scan_string.next();
				scan_string.close();
				System.out.println("Calculating rules.. Please wait..");
				}	
	...
	for(int j=0;j<noOfColumns;j++)
		...
		System.out.println("Output printed to file "+fileName);
		bw.close();
	 }
	...
	//ArrayList<Set<Integer>>possible_numeric_rule= new ArrayList<Set<Integer>>();
	ArrayList<ArrayList<Set<Integer>>>possible_numeric_rule= new ArrayList<ArrayList<Set<Integer>>>();
	Set<String>possible_symbolic_rule_name= new LinkedHashSet<String>();
	Set<String>possible_numeric_rule_name= new LinkedHashSet<String>();
	ArrayList<String>temp_string =new ArrayList<String>();	
	boolean alreadySelectedNumeric = false;
	boolean alreadySelectedSymbolic=false;
	int temp=0;
	int numtemp=0;
	do		
	{			
		...
		{							
					...
					{	
						...
						alreadySelectedSymbolic=false;
						if(checkNumeric.get(j) && possible_numeric_rule.size()!=0){
							if(possible_numeric_rule_name.contains(columnHeading.get(j))){
								int indexofattributeName=0;
								for(String w:possible_numeric_rule_name){
									if(w.equalsIgnoreCase(columnHeading.get(j)))
										break;
									indexofattributeName++;
								}
								ArrayList<Set<Integer>> selectedavpairs= new ArrayList<Set<Integer>>();
								selectedavpairs = possible_numeric_rule.get(indexofattributeName);
								if(selectedavpairs.contains(attributeValuesets.get(j).get(k)))
									alreadySelectedNumeric=true;
								else
									for(Set<Integer> w:selectedavpairs){
										if(set_handler.is_set1_subset_of_set2(w,attributeValuesets.get(j).get(k))
												|| (set_handler.intersection(w,attributeValuesets.get(j).get(k)).size() == 0))
											alreadySelectedNumeric=true;
									}
							}
						}
						else{
							if(possible_symbolic_rule_name.contains(columnHeading.get(j)))
									alreadySelectedSymbolic=true;
						}
						...
						if(current_intersecting_element.size()>current_max_no_of_intersection && current_intersecting_element.size()!=0 && (!possible_rule.contains(attributeValuesets.get(j).get(k)) && !alreadySelectedNumeric && !alreadySelectedSymbolic) )       
								{  
							...
							else if(current_intersecting_element.size()==current_max_no_of_intersection && current_intersecting_element.size()!=0 && (!possible_rule.contains(attributeValuesets.get(j).get(k)) && !alreadySelectedNumeric && !alreadySelectedSymbolic))
								{	
		...
		if(set_handler.is_set1_subset_of_set2(current_intersection, fat_D))
			...
			if(checkNumeric.get(temp_coll)){
				...
				int indexofattributeName=0;
				for(String w:possible_numeric_rule_name){
					if(w.equalsIgnoreCase(columnHeading.get(temp_coll)))
						break;
					indexofattributeName++;
				}
				ArrayList<Set<Integer>> avpairs= new ArrayList<Set<Integer>>();
				if((possible_numeric_rule.size() == indexofattributeName+1)){
					avpairs= possible_numeric_rule.get(indexofattributeName);
					possible_numeric_rule.remove(indexofattributeName);
				}	
				avpairs.add(attributeValuesets.get(temp_coll).get(temp_row));
				possible_numeric_rule.add(indexofattributeName,avpairs);
				numtemp++;
			}
			else{
				possible_symbolic_rule_name.add( columnHeading.get(temp_coll));
			}
		}
		else
		{
